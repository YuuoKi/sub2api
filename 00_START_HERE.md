...
mock 通过只证明内部试运行链路。当前真实复核已获最多图片 4 次、视频 4 次、累计 ¥60 授权，但 2026-07-12 当前 Codex 执行上下文的 presence-check 未检测到凭证变量（未读取值），且该上下文不可见 WSL 发行版，官方镜像又被基础镜像代理 429 阻断；因此尚未产生真实 Provider、真实扣费、资产交付或浏览器截图证据。真实用户上下文曾完成 WSL integration，恢复环境后必须重新检查，不能把上下文差异写成机器无 WSL。
