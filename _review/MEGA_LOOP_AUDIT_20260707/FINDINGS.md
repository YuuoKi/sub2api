...
|----|----------|-------------|-------|--------|
| MLA-P1-001 | `payment_webhook_handler.go:155-157` | `ErrPaymentFulfillmentStale` → HTTP 500 → provider retry storm on stuck RECHARGING | R2, AV3 | **Fixed** (commit `414ff2e9`) |
...
| MLA-P1-005 | `client.ts:165-222` + `auth.ts:168-217` | Dual token refresh race (interceptor vs store timer) | R11, AV8 | **Fixed** (commit `704c8f4c`) |
| MLA-P1-006 | `client.ts:208-214` | Interceptor refresh doesn't sync Pinia store | R11, AV7 | **Fixed** (commit `704c8f4c`) |
| MLA-P1-007 | `PaymentView.vue:792-827` | WeChat JSAPI fallback creates duplicate order | R12, AV3 | **Fixed** (commit `0b8eee6c`; reinforced `42b94ca2`) |
| MLA-P1-008 | `router/index.ts:794,880-884` | Backend mode blocks Stripe return without redirect preservation | R13, AV3 | **Fixed** (commit `2ac31bf5`) |
...
| ID | Location | Description | Round | Status |
|----|----------|-------------|-------|--------|
| MLA-P2-001 | `drama_gateway_service.go:410-429` | Drama list post-filter pagination: wrong totals, sparse pages | R3, AV3 | **Fixed** (commit `35d49031`; reinforced `42b94ca2`) |
...
| MLA-P2-007 | `KeysView.vue:1640+` | Error handling expects Axios shape; interceptor rejects plain object | R14 | **Fixed** (commit `b5350c38`) |
...
| MLA-P2-013 | `.cursor/hooks/secret-scan.ps1:22-29` | Pre-push secret scan fails open when Python/scanner missing (`exit 0`) | R1 Bugbot+Security | **Fixed** (commit `83c71550`) |
