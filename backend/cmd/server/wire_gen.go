...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	gatewayService := service.ProvideGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, identityService, httpUpstream, deferredService, claudeTokenProvider, sessionLimitCache, rpmCache, digestSessionStore, settingService, tlsFingerprintProfileService, channelService, modelPricingResolver, balanceNotifyService, serviceUserPlatformQuotaRepository, generationContentRepository)
	...
	videoGatewayRepository := repository.NewVideoGatewayRepository(db)
	videoKeyEncryptor, err := repository.NewVideoKeyEncryptor(configConfig)
	if err != nil {
		return nil, err
	}
	realCreateGuard := service.ProvideRealCreateGuard(configConfig)
	providerRealAccessPolicyRepository := repository.NewProviderRealAccessPolicyRepository(db)
	videoGatewayService := service.ProvideVideoGatewayServiceWithContentCollector(videoGatewayRepository, videoKeyEncryptor, configConfig, userRepository, settingService, billingCacheService, generationContentRepository, realCreateGuard, providerRealAccessPolicyRepository)
	realReviewSessionHandler := admin.NewRealReviewSessionHandler(videoGatewayService)
	reliabilityReconciliationSource := repository.NewReliabilityReconciliationRepository(db)
	reliabilityMetrics := service.ProvideReliabilityMetrics()
	reliabilityReconciler := service.ProvideReliabilityReconciler(reliabilityReconciliationSource, reliabilityMetrics)
	videoHandler := handler.ProvideAdminVideoHandler(videoGatewayService, reliabilityReconciler)
	generationContentHandler := handler.ProvideAdminGenerationContentHandler(generationContentRepository, configConfig, settingService)
	providerBillingStore := repository.NewProviderBillingRepository(db)
	providerBillingService := service.ProvideProviderBillingService(providerBillingStore)
	providerBillingHandler := admin.NewProviderBillingHandler(providerBillingService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler, realReviewSessionHandler, videoHandler, generationContentHandler, providerBillingHandler)
	...
	batchImagePublicService := service.ProvideBatchImagePublicService(batchImageRepository, accountRepository, groupRepository, userGroupRateRepository, batchImageQueue, batchImageModelPricingResolver, usageBillingRepository, apiKeyAuthCacheInvalidator, configConfig, realCreateGuard, settingService, providerRealAccessPolicyRepository)
	...
	handlerVideoHandler := handler.NewVideoHandler(videoGatewayService)
	generationContentAdoptionService := service.NewGenerationContentAdoptionService(videoGatewayService, generationContentRepository, configConfig)
	handlerGenerationContentHandler := handler.NewGenerationContentHandler(generationContentAdoptionService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	videoGatewayWorker := service.ProvideVideoGatewayWorker(videoGatewayService, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, batchImageHandler, handlerVideoHandler, handlerGenerationContentHandler, idempotencyCoordinator, idempotencyCleanupService, videoGatewayWorker)
	...
	batchImageWorkerRuntime := service.ProvideBatchImageWorkerRuntime(batchImageRepository, accountRepository, batchImageQueue, usageBillingRepository, usageLogRepository, batchImageModelPricingResolver, apiKeyAuthCacheInvalidator, configConfig, providerRealAccessPolicyRepository)
	...
	domainOutboxRepository := repository.NewDomainOutboxRepository(db)
	videoOutboxHandlers := service.ProvideVideoOutboxHandlers(videoGatewayService, domainOutboxRepository, billingCacheService, balanceNotifyService)
	domainOutboxWorker := service.ProvideDomainOutboxWorker(domainOutboxRepository, videoOutboxHandlers, configConfig)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, schedulerSnapshotService, tokenRefreshService, accountExpiryService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, userPlatformQuotaUsageFlusher, generationContentRetentionService, videoGatewayWorker, domainOutboxWorker)
