...
type adminPasswordResetter interface {
	ResetUserPassword(ctx context.Context, id int64) (*service.InitialCredential, error)
}
...
type CreateUserResponse struct {
	...
	InitialCredential *service.InitialCredential `json:"initial_credential"`
}
...
func (h *UserHandler) Create(c *gin.Context) {
	...
	if len(req.Password) > 0 {
		response.BadRequest(c, "Password is generated by the server and cannot be supplied")
		return
	}
	...
	response.Success(c, CreateUserResponse{
		User:              dto.UserFromServiceAdmin(user),
		InitialCredential: user.InitialCredential,
	})
}

// ResetPassword replaces a user's password with a one-time temporary credential.
...
func (h *UserHandler) ResetPassword(c *gin.Context) {
	...
	resetter, ok := h.adminService.(adminPasswordResetter)
	if !ok {
		response.InternalError(c, "Password reset is not available")
		return
	}
	credential, err := resetter.ResetUserPassword(c.Request.Context(), userID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{"initial_credential": credential})
}
...
func (h *UserHandler) Update(c *gin.Context) {
	...
	if len(req.Password) > 0 {
		response.BadRequest(c, "Use the one-time password reset action instead")
		return
	}
