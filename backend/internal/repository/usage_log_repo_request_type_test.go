...
func TestUsageLogRepositoryGetUserSpendingRanking(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "actual_cost", "requests", "tokens", "member_type", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(2), "beta@example.com", 12.5, int64(9), int64(900), "tool", 40.0, int64(30), int64(2600)).
		AddRow(int64(1), "alpha@example.com", 12.5, int64(8), int64(800), "human", 40.0, int64(30), int64(2600)).
		AddRow(int64(3), "gamma@example.com", 4.25, int64(5), int64(300), "human", 40.0, int64(30), int64(2600))

	mock.ExpectQuery("(?s)WITH user_spend AS \\(.*us\\.notes.*[Aa][Ss] member_type.*GROUP BY u\\.user_id, us\\.email, us\\.notes.*SELECT\\s+user_id,\\s+email,\\s+actual_cost,\\s+requests,\\s+tokens,\\s+member_type,\\s+total_actual_cost,\\s+total_requests,\\s+total_tokens\\s+FROM ranked").
	...
	require.Equal(t, &usagestats.UserSpendingRankingResponse{
		Ranking: []usagestats.UserSpendingRankingItem{
			{UserID: 2, Email: "beta@example.com", ActualCost: 12.5, Requests: 9, Tokens: 900, MemberType: "tool"},
			{UserID: 1, Email: "alpha@example.com", ActualCost: 12.5, Requests: 8, Tokens: 800, MemberType: "human"},
			{UserID: 3, Email: "gamma@example.com", ActualCost: 4.25, Requests: 5, Tokens: 300, MemberType: "human"},
		},
