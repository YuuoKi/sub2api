...
func (r *usageLogRepository) GetUserSpendingRanking(ctx context.Context, startTime, endTime time.Time, limit int) (result *UserSpendingRankingResponse, err error) {
	...
	query := `
		WITH user_spend AS (
			SELECT
				...
				COALESCE(us.email, '') as email,
				CASE
					WHEN regexp_replace(COALESCE(us.notes, ''), '^[[:space:]]+', '') LIKE '[工具]%' THEN 'tool'
					ELSE 'human'
				END as member_type,
				COALESCE(SUM(u.actual_cost), 0) as actual_cost,
				COUNT(*) as requests,
			...
			GROUP BY u.user_id, us.email, us.notes
		),
		ranked AS (
			SELECT
				user_id,
	...
	for rows.Next() {
		...
		if err = rows.Scan(&row.UserID, &row.Email, &row.ActualCost, &row.Requests, &row.Tokens, &row.MemberType, &totalActualCost, &totalRequests, &totalTokens); err != nil {
			return nil, err
		}
