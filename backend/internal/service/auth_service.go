...
var (
	...
	ErrTemporaryPasswordExpired = infraerrors.Forbidden("TEMPORARY_PASSWORD_EXPIRED", "temporary password has expired; ask an administrator to reset it")
)
...
func (s *AuthService) Login(ctx context.Context, email, password string) (string, *User, error) {
	...
	if user.MustChangePassword && (user.TemporaryPasswordExpiresAt == nil || !time.Now().UTC().Before(*user.TemporaryPasswordExpiresAt)) {
