...
vi.mock('vue-i18n', async () => {
  ...
  const messages: Record<string, string> = {
    ...
    'admin.generationContent.exampleBannerTitle': 'No captured content yet',
    'admin.generationContent.exampleBannerDesc': 'Real redacted samples will appear here once capture has data.'
  }
  return {
    ...actual,
    useI18n: () => ({
...
describe('ContentWall adoption feedback', () => {
  ...
  it('shows the underlying API error message when the save request fails', async () => {
    ...
    expect(wrapper.emitted('updated')).toBeUndefined()
    // extractApiErrorMessage surfaces the real error detail instead of a generic fallback string.
    expect(wrapper.text()).toContain('network down')
  })

  it('falls back to the generic message when the rejection carries no usable detail', async () => {
    updateAdoption.mockRejectedValue({})
    const wrapper = mountWall()
  ...
  it('renders an honest empty state (no example/demo fixture wording) when nothing has been captured', () => {
    const emptyWrapper = mount(ContentWall, { props: { isLive: false, samples: [] } })
    expect(emptyWrapper.text()).toContain('No captured content yet')
    expect(emptyWrapper.text().toLowerCase()).not.toContain('example')
    expect(emptyWrapper.text().toLowerCase()).not.toContain('demo')
  })
})
