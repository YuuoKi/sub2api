<template>
  ...
  >
    ...
    <p class="mt-5 text-xs font-medium tracking-wide text-ui-accent" data-testid="empty-state-brand">
      {{ productName }}
    </p>
    <h3 class="mt-2 text-base font-semibold text-ui-text">{{ title }}</h3>
...
export type AnimatedEmptyStateVariant = 'video-tasks' | 'generic'
...
const productName = computed(() => resolveProductName(props.brandName || DEFAULT_PRODUCT_NAME))
