export default {
    accounts: {
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非本平台配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在本平台端解除该限制。',
      ...
      refreshToken: '刷新令牌',
      noAccountsYet: '暂无账号',
      emptyPrerequisite: '前置条件：先创建分组，再添加并绑定上游账号。没有分组时不能完成账号接入。',
      createGroupFirst: '先创建分组',
      addUpstreamAccount: '添加上游账号',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个本平台实例的场景。',
