  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        <span v-else class="badge badge-gray">{{ t('admin.generationContent.notLive') }}</span>
      </div>
...
// 只用后端 is_live 信号；不因为样本数组非空就强行判定为“真实数据”，避免伪造活跃状态。
const isLive = computed(() => Boolean(stats.value?.is_live || samplesLive.value))
...
const load = async () => {
  ...
  if (samplesResult.status === 'fulfilled') {
    ...
    samplesLive.value = Boolean(samplesResult.value.is_live)
