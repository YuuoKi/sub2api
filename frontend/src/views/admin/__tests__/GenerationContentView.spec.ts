...
vi.mock('vue-i18n', async () => {
  ...
  const messages: Record<string, string> = {
    ...
    'admin.generationContent.notLive': 'Not live yet',
...
describe('GenerationContentView', () => {
  ...
  it('does not fabricate a live state just because samples are non-empty (requires backend is_live)', async () => {
    getStats.mockResolvedValue({
      captured_today: 0,
      captured_week: 0,
    ...
    expect(wrapper.find('[data-test="content-wall"]').text()).toContain('live=false')
  })
