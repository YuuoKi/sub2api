  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex flex-col gap-3 border-b border-gray-200 pb-4 dark:border-dark-700 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Video Tasks</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Recent task status across video providers</p>
        </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
              <tr v-for="task in tasks" :key="task.id" class="hover:bg-gray-50 dark:hover:bg-dark-700/40">
                <td class="px-5 py-3">
                  ...
                  <div v-if="task.error_message" class="mt-1 max-w-xl truncate text-xs text-red-600 dark:text-red-300">{{ task.error_message }}</div>
                </td>
                <td class="px-5 py-3">
              ...
              <tr v-if="!loading && !tasks.length">
                <td colspan="5" class="px-5 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No tasks</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="flex items-center justify-between border-t border-gray-200 px-5 py-4 text-sm dark:border-dark-700">
          <div class="text-gray-500 dark:text-gray-400">Total {{ pagination.total }}</div>
          <div class="flex gap-2">
            <button class="btn btn-sm btn-outline" type="button" :disabled="pagination.page <= 1" @click="changePage(pagination.page - 1)">Prev</button>
            <span class="flex items-center px-2 text-gray-700 dark:text-gray-200">{{ pagination.page }} / {{ pagination.pages }}</span>
            <button class="btn btn-sm btn-outline" type="button" :disabled="pagination.page >= pagination.pages" @click="changePage(pagination.page + 1)">Next</button>
          </div>
        </div>
      </section>
    </div>
...
async function loadTasks() {
  ...
  try {
    ...
    appStore.showError(extractApiErrorMessage(err, 'Failed to load video tasks'))
  } finally {
    loading.value = false
  }
}
